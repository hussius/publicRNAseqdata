...
```{r:loadingsPC1-3logCPM}
    ...
    p <- prcomp(t(voom))
     ...
     load.pc2 <- p$rotation[,2][order(p$rotation[,2])]
      ...
      barplot(extreme.pc2, names.arg=symbs, las=2, main="Genes w highest absolute loadings in PC2 (log-CPM)")
     ...
     load.pc3 <- p$rotation[,3][order(p$rotation[,3])]
      ...
      barplot(extreme.pc3, names.arg=symbs, las=2, main="Genes w highest absolute loadings in PC3 (log-CPM)")
```
