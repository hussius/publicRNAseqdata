...
plot.pca.published <- function(df,x,y,z){
  ...
  plot(p$x[,x],p$x[,y],pch=shapes,cex=1.5,col=colors,xlab=paste("PC1",round(v.x),"% of variance"),ylab=paste("PC2",round(v.y),"% of variance"),main=paste(z," Published FPKM/RPKM values \n n=13,323"))
...
plot.pca.reprocessed <- function(df,x,y,z){
  ...
  plot(p_repr$x[,x],p_repr$x[,y],pch=shapes_repr,col=colors_repr,xlab=paste("PC1",round(v.x),"% of variance"),ylab=paste("PC2",round(v.y),"% of variance"),main=paste(z," reprocessed  cufflinks FPKM values \n n=19,475"))
...
merged_data_co <- merged_data[-which(rowSums(merged_data)<=0.01),]                       

#-----------DOWNLOAD REPROCESSED DATA-----------
reprocessed_data <- read.table("fpkm_table_tophat.txt",sep="\t",header=T)

# FUNKTIONEN getPcs finns redan där uppe!
...
reprocessed_data_pc1 <- getPcs(reprocessed_data)
reprocessed_data_pc <- data.matrix(reprocessed_data_pc1[,2:16])

reprocessed_data_pc_co <- reprocessed_data_pc[-which(rowSums(reprocessed_data_pc)<=0.01),]
...
merg_altIso_heart <- get_data(merged_data_co,"heart","AltIso",2)
merg_atlas_heart <- get_data(merged_data_co,"heart","Atlas",2)
merg_gtex_heart <- get_data(merged_data_co,"heart","GTEx",2)
merg_HPA_heart <- get_data(merged_data_co,"heart","HPA",2)

merg_altIso_brain <- get_data(merged_data_co,"brain","AltIso",2)
merg_atlas_brain <- get_data(merged_data_co,"brain","Atlas",2)
merg_gtex_brain <- get_data(merged_data_co,"brain","GTEx",2)
merg_HPA_brain <- get_data(merged_data_co,"brain","HPA",2)

merg_atlas_kidney <- get_data(merged_data_co,"kidney","Atlas",2)
merg_gtex_kidney <- get_data(merged_data_co,"kidney","GTEx",2)
merg_HPA_kidney <- get_data(merged_data_co,"kidney","HPA",2)
...
repr_altIso_heart <- get_data(reprocessed_data_pc_co,"heart","AltIso")
repr_atlas_heart <- get_data(reprocessed_data_pc_co,"heart","Atlas")
repr_Bodymap_heart <- get_data(reprocessed_data_pc_co,"heart","BodyMap")
repr_EoGE_heart <- get_data(reprocessed_data_pc_co,"heart","EoGE")
repr_HPA_heart <- get_data(reprocessed_data_pc_co,"heart","HPA")

repr_altIso_brain <- get_data(reprocessed_data_pc_co,"brain","AltIso")
repr_atlas_brain <- get_data(reprocessed_data_pc_co,"brain","Atlas")
repr_Bodymap_brain <- get_data(reprocessed_data_pc_co,"brain","BodyMap")
repr_EoGE_brain <- get_data(reprocessed_data_pc_co,"brain","EoGE")
repr_HPA_brain <- get_data(reprocessed_data_pc_co,"brain","HPA")

repr_atlas_kidney <- get_data(reprocessed_data_pc_co,"kidney","Atlas")
repr_Bodymap_kidney <- get_data(reprocessed_data_pc_co,"kidney","BodyMap")
repr_EoGE_kidney <- get_data(reprocessed_data_pc_co,"kidney","EoGE")
repr_HPA_kidney <- get_data(reprocessed_data_pc_co,"kidney","HPA")
...
published.nozero.qn <- quantile_normalisation(published.nozero)
published.log.qn <- quantile_normalisation(published.log)



pheatmap(cor(published.nozero.qn, method="spearman")) 
...
p <- prcomp(t(published.log.qn))
shapes <- c(rep(15,3),rep(16,2),rep(17,3),rep(8,3))
plot(p$x[,1],p$x[,2],pch=shapes,cex=1.5,col=colors,xlab=paste("PC1 58% of variance"),ylab=paste("PC2 13% of variance"),main="Quantile normalized Published FPKM/RPKM values \n n=13,323")
legend("bottomleft",legend=c("Heart","Brain","Kidney"),col=c("indianred", "dodgerblue", "forestgreen"),cex=1.5,pch=20)
legend("top",legend=c("HPA","AltIso","GTEx","Atlas"),col="black",pch=c(15,16,17,8),ncol=2)

do_anova(published.nozero.qn,sampleinfo_published,"ANOVA, published data")
