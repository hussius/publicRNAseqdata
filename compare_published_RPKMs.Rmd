...
fit <- lm(RPKM ~ nraw + layout + prep + study + tissue, data=data)
a <- anova(fit)
maxval = 3000
```
...
data <- data.frame(m, tissue=tissue, study=study, prep=prep, layout=layout,nraw=raw,nmapped=mapped)
#subset <- data[sample(1:nrow(data), 1000),]
fit <- lm(log2RPKM ~ nraw + layout + prep + study + tissue, data=data)
b <- anova(fit)
...
data <- data.frame(m, tissue=tissue, study=study, prep=prep, layout=layout,nraw=raw,nmapped=mapped)
fit <- lm(combat ~ nraw  + layout + prep + study + tissue, data=data)
c <- anova(fit)
