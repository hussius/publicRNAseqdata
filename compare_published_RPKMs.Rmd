...
#write.table(flipped, file="transposed_published_rpkms.txt", col.names=F, row.names=F, quote=F, sep=",")
```
...
flipped <- t(fpkms.log)
write.table(flipped, file="transposed_published_rpkms_log.txt", col.names=F, row.names=F, quote=F, sep=",")
...
```{r:prcomp1-3log}
  #    symbol <- read.table("ENSG_Symbol.txt",header=TRUE)
    ...
    textxy(p$x[,1],p$x[,2],labs=rownames(p$x))
      ...
      barplot(extreme.pc1, names.arg=symbs, las=2, main="Genes w highest absolute loadings in PC1 (logRPKM)")
      ...
      barplot(extreme.pc2, names.arg=symbs, las=2, main="Genes w highest absolute loadings in PC2 (logRPKM)")
      ...
      barplot(extreme.pc3, names.arg=symbs, las=2, main="Genes w highest absolute loadings in PC3 (logRPKM)")

loadings.3 <- p$x[,1:3]
write.table(loadings.3,file="loadings_pub_log.txt",sep=",",quote=F,col.names=F,row.names=F)
```
...
HPA_b <- rownames(f[order(f$HPA_brain,decreasing=TRUE),][1:100,])
HPA_h <- rownames(f[order(f$HPA_heart,decreasing=TRUE),][1:100,])
HPA_k <- rownames(f[order(f$HPA_kidney,decreasing=TRUE),][1:100,])

AltIso_b <- rownames(f[order(f$AltIso_brain,decreasing=TRUE),][1:100,])
AltIso_h <- rownames(f[order(f$AltIso_heart,decreasing=TRUE),][1:100,])

GTEx_b <- rownames(f[order(f$GTEx_brain,decreasing=TRUE),][1:100,])
GTEx_h <- rownames(f[order(f$GTEx_heart,decreasing=TRUE),][1:100,])
GTEx_k <- rownames(f[order(f$GTEx_kidney,decreasing=TRUE),][1:100,])

Atlas_b <- rownames(f[order(f$Atlas_brain,decreasing=TRUE),][1:100,])
Atlas_h <- rownames(f[order(f$Atlas_heart,decreasing=TRUE),][1:100,])
Atlas_k <- rownames(f[order(f$Atlas_kidney,decreasing=TRUE),][1:100,])
...
HPA_b_c <- rownames(combat[order(combat[,1],decreasing=TRUE),][1:100,])
HPA_h_c <- rownames(combat[order(combat[,2],decreasing=TRUE),][1:100,])
HPA_k_c <- rownames(combat[order(combat[,3],decreasing=TRUE),][1:100,])

AltIso_b_c <- rownames(combat[order(combat[,4],decreasing=TRUE),][1:100,])
AltIso_h_c <- rownames(combat[order(combat[,5],decreasing=TRUE),][1:100,])

GTEx_b_c <- rownames(combat[order(combat[,6],decreasing=TRUE),][1:100,])
GTEx_h_c <- rownames(combat[order(combat[,7],decreasing=TRUE),][1:100,])
GTEx_k_c <- rownames(combat[order(combat[,8],decreasing=TRUE),][1:100,])

Atlas_b_c <- rownames(combat[order(combat[,9],decreasing=TRUE),][1:100,])
Atlas_h_c <- rownames(combat[order(combat[,10],decreasing=TRUE),][1:100,])
Atlas_k_c <- rownames(combat[order(combat[,11],decreasing=TRUE),][1:100,])
```
Brain:
```{r:combatVennBrain}
